name 'every subproperty axiom is essential'
...
query %q{
  ...
  where {
    graph ?graph { ?sub rdfs:subPropertyOf ?sup }
  }
...
filter do |r, emit|
  if r
    ...
    # Accumulate asserted subproperty axioms and their graphs.
    ...
    # At end of input, calculate inferred subproperty axioms.
...
case_name { |r| "#{r.sub.to_s} subproperty of #{r.sup.to_s}" }